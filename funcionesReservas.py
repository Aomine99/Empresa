...
def listarCheckInCheckOut(habitacion):
    """
    Carga todos los datos de las reservas en la bd
    :return:
        Retorna una variable con todos los datos cargados
    ...
    try:
        conexion.cur.execute('select checkIn, checkOut from reservas where numHabitacion = ? order by checkIn',(habitacion,))
        listadoFechas = conexion.cur.fetchall()
        conexion.conex.commit()
...
def comprobarFechas(habitacion, checkIn, checkOut):
    lista = listarCheckInCheckOut(habitacion)

    fechas = []

    contador_bien = 0

    dateCheckIn = datetime.strptime(checkIn,'%d/%m/%Y')
    dateCheckOut = datetime.strptime(checkOut,'%d/%m/%Y')

    for i in range (len(lista)):
        entrada = datetime.strptime(lista[i][0], '%d/%m/%Y')
        salida = datetime.strptime(lista[i][1], '%d/%m/%Y')

        fechas.append((entrada, salida))
        fechas.sort(key=lambda fecha: fecha[0])

    for i in range(len(fechas)):
        if dateCheckIn < fechas[i][0]:
            if dateCheckOut <= fechas[i][0]:
                contador_bien = contador_bien +1

        elif dateCheckIn >= fechas[i][1]:
            i = i + 1
            contador_bien = contador_bien + 1

    if contador_bien == len(fechas):
        return True
    else:
        return False

def comprobar_existe(dni, habitacion, checkIn, checkOut):
    try:
        conexion.cur.execute('Select * from reservas where dni = ? and numHabitacion = ? and checkIn = ? and checkOut = ?',(dni, habitacion, checkIn, checkOut,))
        listado = conexion.cur.fetchall()
        conexion.conex.commit()
        return listado
