...
def modificarHab(registro):
    ...
    try:
        ...
        conexion.cur.execute("update habitaciones set numeroHabitacion = ?, tipo = ?, precio = ?, ocupado = ? where numeroHabitacion = ?",(registro[0],registro[1],registro[2], registro[3], registro[0]))
        conexion.conex.commit()
...
def cambiaestadohab(libre, numhabres):
    """
    Cambia el estado de libre a ocupado en las habitaciones
    :param libre:
        Contiene el valor "Si" o "No" dependiendo de cual queramos asignarle
    :param numhabres:
        Contiene el numero de la habitacion asociado a la reserva
    :return:
        No retorna
    """
